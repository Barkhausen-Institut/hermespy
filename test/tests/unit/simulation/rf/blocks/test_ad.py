...
class TestDAC(TestConverterBase):
    ...
    def test_init_validation(self) -> None:
        """DAC initialization should raise ValueErrors on invalid arguments"""

        with self.assertRaises(ValueError):
            _ = DAC(reference_impedance=-1.234)

    def test_max_output_power_setget(self) -> None:
        """Max output power property getter should return setter argument"""

        expected_output_power = 0.1234
        self.converter.max_output_power = expected_output_power
        self.assertEqual(self.converter.max_output_power, expected_output_power)

    def test_max_output_power_validation(self) -> None:
        """Max output power property setter should raise ValueError on invalid arguments"""

        with self.assertRaises(ValueError):
            self.converter.max_output_power = -1.1234
