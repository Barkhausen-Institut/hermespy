...
from hermespy.simulation.rf.noise.phase_noise import NoPhaseNoise, OscillatorPhaseNoise
from hermespy.simulation import SimulatedDevice, RFSignal
...
from ....utils import assert_signals_equal
...
class TestNoPhaseNoise(TestCase):
    ...
    def test_add_noise(self) -> None:
        ...
        signal = RFSignal.FromNDArray(np.random.standard_normal((3, 10)), 1)
        noisy_signal = self.pn.add_noise(signal)
        assert_signals_equal(self, signal, noisy_signal)
...
class TestOscillatorPhaseNoise(TestCase):
    ...
    def test_add_noise(self) -> None:
        ...
        operator.waveform = RootRaisedCosineWaveform(num_preamble_symbols=0, num_data_symbols=40, roll_off=0.9)

        transmission = operator.transmit(device.state())
        signal = transmission.signal
        noise_realization = self.pn.realize(device.bandwidth, device.oversampling_factor)
        noisy_signal = noise_realization.add_noise(signal)

        num_samples = signal.num_samples
        sampling_rate = signal.sampling_rate

        pn_samples = noise_realization._get_noise_samples(num_samples, 1, sampling_rate)

        # check if noised signal magnitude is the same as in the original signal
        clear_signal_avg_amp = np.average(np.abs(signal.view(np.ndarray)), axis=1)
        noisy_signal_avg_amp = np.average(np.abs(noisy_signal.view(np.ndarray)), axis=1)
        ...
        arg_diffs = np.angle(noisy_signal.view(np.ndarray)) - np.angle(pn_samples)
        arg_signal = np.angle(signal.view(np.ndarray))
