...
class SelectiveLeakageCalibration(LeakageCalibrationBase, Serializable):
    ...
    def __probe_leakage(
        ...
        for p, n in np.ndindex(num_probes, device.antennas.num_transmit_ports):
            ...
            received_waveforms[p, :, n, :] = rx_signal.samples[:, start:start + num_wavelet_samples]
    ...
    def LeastSquaresEstimate(
        ...
        if filter_calibration:
            ...
            peaks: np.ndarray = find_peaks(summed_response, .25 * summed_response.max())[0]
            if peaks.size != 0:
                widths: np.ndarray = np.ceil(peak_widths(summed_response, peaks, rel_height=.75)[0])
                ...
                for p, w in zip(peaks, widths):
                    ...
                    new_leakage_response[:, :, p_start:p_end] = leakage_response[:, :, p_start:p_end]
