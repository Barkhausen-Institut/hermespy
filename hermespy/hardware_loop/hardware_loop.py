...
from hermespy.core import Drop, Evaluation, Evaluator, MonteCarloResult, Pipeline, Operator
...
class HardwareLoop(Generic[PhysicalScenarioType], Pipeline[PhysicalScenarioType]):
    ...
    def __run(self) -> np.ndarray:
            ...
            for indices in np.ndindex(grid):
                ...
                try:
                    ...
                    progress.log((str(total), str(indices[0]),
                                  *[str(d.sample_points[i]) for d, i in zip(self.__dimensions, indices[1:])],
        ...
        result = MonteCarloResult(self.__dimensions, self.__evaluators, samples, 0.)
