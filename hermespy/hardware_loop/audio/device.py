...
class AudioDevice(PhysicalDevice, Serializable):
    ...
    __playback_device: int                  # Device over which audio streams are to be transmtited
    __record_device: int                    # Device over which audio streams are to be received
    __playback_channels: np.ndarray         # List of audio channel for signal transmission
    __record_channels: np.ndarray           # List of audio channel for signal reception
    ...
    def playback_channels(self, value: Union[np.ndarray, Iterable]):
        ...
        if value.ndim != 1:
            raise ValueError("Number of channels must be a vector (a one-dimensional array)")
    ...
    def _upload(self, signal: Signal) -> None:
        ...
        #resample = signal.resample(self.sampling_rate)
        #mixed_samples = np.real(resample.samples * np.exp(1j * np.pi * resample.timestamps * self.sampling_rate))
        #appended_samples = np.append(mixed_samples, np.zeros((mixed_samples.shape[0], delay_samples), dtype=float), axis=1)
        
        resampled_samples = signal.resample(self.sampling_rate).samples
        pressure_signal = np.roll(fft(resampled_samples), int(.25*resampled_samples.shape[1]), axis=1)
