...
class DetectionProbEvaluator(ScalarEvaluator, Serializable):
    ...
    def __init__(
        ...
        """
        Args:
            radar: Radar detector to be evaluated.
...
class RocEvaluationResult(EvaluationResult):
    ...
    def __init__(
        ...
        # Initialize class attributes
        self.__artifacts = np.empty([d.num_sample_points for d in grid], dtype=object)
        for indices in np.ndindex(self.__artifacts.shape):
            self.__artifacts[indices] = []
    ...
    def to_array(self) -> np.ndarray:
        probabilities = np.empty((*self.__artifacts.shape, self.__num_thresholds, 2), dtype=float)
...
class RootMeanSquareErrorResult(EvaluationResult):
    ...
    def __init__(
        ...
        # Initialize class attributes
        grid_dimensions = tuple(g.num_sample_points for g in grid)
        self.__cummulative_errors = np.zeros(grid_dimensions, dtype=float)
        self.__num_errors = np.zeros(grid_dimensions, dtype=int)
    ...
    @override
    def to_array(self) -> np.ndarray:
        return np.sqrt(self.__cummulative_errors / self.__num_errors)


class RootMeanSquareError(RadarEvaluator, Serializable):
    ...
    @override
    def evaluate(self) -> RootMeanSquareEvaluation:
    ...
    @override
    def initialize_result(self, grid: Sequence[GridDimensionInfo]) -> RootMeanSquareErrorResult:
        return RootMeanSquareErrorResult(grid, self)
