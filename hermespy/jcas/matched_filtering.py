...
class MatchedFilterJcas(DuplexModem, Radar):
    ...
    def receive(self) -> JCASReception:
        ...
        required_num_received_samples = self.transmission.signal.num_samples + \
            num_propagated_samples
        if signal.num_samples < required_num_received_samples:
            signal.append_samples(Signal(np.zeros((1, required_num_received_samples -
                                  signal.num_samples), dtype=complex), self.sampling_rate, signal.carrier_frequency))

        # Remove possible overhead samples if signal is too long
        # resampled_signal.samples = resampled_signal.samples[:, :num_samples]

        correlation = abs(correlate(signal.samples, self.transmission.signal.samples,
                          mode='valid', method='fft').flatten()) / self.transmission.signal.num_samples
        ...
        cube_data = np.array(
            [[correlation[:num_propagated_samples]]], dtype=float)
