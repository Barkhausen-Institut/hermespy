...
from hermespy.core import ChannelStateInformation, ChannelStateFormat, Device, Direction, FloatingError, HDFSerializable, Moveable, Serializable, Signal, Transformable, Transformation
...
class RadarChannelRealization(ChannelRealization):
    ...
    def _propagate(self, signal: Signal, transmitter: Device, receiver: Device, interpolation: InterpolationMode) -> Signal:
        delays = np.array([path_realization.propagation_delay(transmitter, receiver) for path_realization in self._path_realizations])
        ...
        if delays.size > 0:
            max_delay_in_samples = ceil(delays.max() * signal.sampling_rate + 2 * velocities.max() * signal.num_samples / (signal.sampling_rate * speed_of_light))
        else:
            max_delay_in_samples = 0

        propagated_samples = np.zeros((receiver.antennas.num_receive_antennas, signal.num_samples + max_delay_in_samples), dtype=np.complex_)
        ...
        for path_realization, delay, velocity in zip(self._path_realizations, delays, velocities):
            path_realization.add_propagation(transmitter, receiver, signal, propagated_samples, delay, velocity)
    ...
    def state(self, transmitter: Device, receiver: Device, delay: float, sampling_rate: float, num_samples: int, max_num_taps: int) -> ChannelStateInformation:
        raw_state = np.zeros((receiver.antennas.num_receive_antennas, transmitter.antennas.num_transmit_antennas, num_samples, max_num_taps), dtype=np.complex_)
        ...
        return ChannelStateInformation(ChannelStateFormat.IMPULSE_RESPONSE, GCXS.from_numpy(raw_state))
...
class RadarChannelBase(Generic[RCRT], Channel[RCRT]):
    ...
    def _realize_target(self, target: RadarTarget) -> RadarTargetRealization:
        ...
        target_impinging_direction = target_backwards_transform.transform_direction(self.alpha_device.global_position, normalize=True)
        target_emerging_direction = target_backwards_transform.transform_direction(self.beta_device.global_position, normalize=True)
        ...
        return RadarTargetRealization(target_forwards_transform.translation, target.get_velocity(), cross_section, reflection_phase, self.attenuate, target.static)
...
class RadarPathRealization(HDFSerializable):
    ...
    @abstractmethod
    def propagation_response(self, transmitter: Device, receiver: Device, carrier_frequency: float) -> np.ndarray:
    ...
    def add_propagation(self, transmitter: Device, receiver: Device, signal: Signal, propagated_samples: np.ndarray, propagation_delay: float | None = None, relative_velocity: float | None = None) -> None:
        ...
        propagation_delay = self.propagation_delay(transmitter, receiver) if propagation_delay is None else propagation_delay
        relative_velocity = self.relative_velocity(transmitter, receiver) if relative_velocity is None else relative_velocity
        propagation_response = self.propagation_response(transmitter, receiver, signal.carrier_frequency)
        ...
        propagated_samples[:, delay_sample_offset : delay_sample_offset + signal.num_samples] += np.einsum("ij,jk,k->ik", propagation_response, signal.samples, echo_weights)

    def add_state(self, transmitter: Device, receiver: Device, delay: float, sampling_rate: float, state: np.ndarray) -> None:
        ...
        propagation_response = self.propagation_response(transmitter, receiver, transmitter.carrier_frequency)
        ...
        state[:, :, :, delay_sample_offset] += np.einsum("ij,k->ijk", propagation_response, echo_weights)
...
class RadarTargetRealization(RadarPathRealization):
    ...
    def __init__(self, position: np.ndarray, velocity: np.ndarray, cross_section: float, reflection_phase: float, attenuate: bool = True, static: bool = False) -> None:
        """
        Args:
    ...
    def propagation_delay(self, transmitter: Device, receiver: Device) -> float:
        ...
        delay = (np.linalg.norm(emerging_vector) + np.linalg.norm(impinging_vector)) / speed_of_light
        return delay
    ...
    def relative_velocity(self, transmitter: Device, receiver: Device) -> float:
        ...
        relative_transmitter_velocity = np.dot(Direction.From_Cartesian(emerging_vector, normalize=True), target_velocity - transmitter.velocity)
        relative_receiver_velocity = np.dot(Direction.From_Cartesian(impinging_vector, normalize=True), receiver.velocity - target_velocity)

        return relative_transmitter_velocity + relative_receiver_velocity

    def propagation_response(self, transmitter: Device, receiver: Device, carrier_frequency: float) -> np.ndarray:
        # Query the sensor array responses
        rx_response = receiver.antennas.cartesian_array_response(carrier_frequency, self.position, "global").conj()
        tx_response = transmitter.antennas.cartesian_array_response(carrier_frequency, self.position, "global")
        ...
        if self.attenuate:
            ...
            amplitude_factor = wavelength * self.cross_section**0.5 / ((4 * pi) ** 1.5 * tx_distance * rx_distance)

        else:
            amplitude_factor = 1.0

        # Compute the MIMO response
        return amplitude_factor * np.exp(1j * self.reflection_phase) * np.inner(rx_response, tx_response)
    ...
    def from_HDF(cls: Type[RadarTargetRealization], group: Group) -> RadarTargetRealization:
        ...
        return RadarTargetRealization(position, velocity, cross_section, reflection_phase, **parameters)
...
class RadarInterferenceRealization(RadarPathRealization):
    ...
    def propagation_delay(self, transmitter: Device, receiver: Device) -> float:
        delay = np.linalg.norm(receiver.global_position - transmitter.global_position) / speed_of_light
        return delay

    def relative_velocity(self, transmitter: Device, receiver: Device) -> float:
        connection = Direction.From_Cartesian(receiver.global_position - transmitter.global_position, normalize=True).view(np.ndarray)
        return np.dot(transmitter.velocity - receiver.velocity, connection)

    def propagation_response(self, transmitter: Device, receiver: Device, carrier_frequency: float) -> np.ndarray:
        # Model the sensor arrays' spatial responses
        rx_response = receiver.antennas.cartesian_array_response(carrier_frequency, transmitter.global_position, "global").conj()
        tx_response = transmitter.antennas.cartesian_array_response(carrier_frequency, receiver.global_position, "global")
    ...
    @classmethod
    def from_HDF(cls: Type[RadarInterferenceRealization], group: Group) -> RadarInterferenceRealization:
        # Deserialize base class
        parameters = RadarPathRealization._parameters_from_HDF(group)
...
class SingleTargetRadarChannelRealization(RadarChannelRealization):
    ...
    def __init__(self, alpha_device: Device, beta_device: Device, gain: float, target_realization: RadarTargetRealization | None, interpolation_mode: InterpolationMode = InterpolationMode.NEAREST) -> None:
        """
        Args:
    ...
    def null_hypothesis(self) -> SingleTargetRadarChannelRealization:
        return SingleTargetRadarChannelRealization(self.alpha_device, self.beta_device, self.gain, None, self.interpolation_mode)

    def ground_truth(self) -> np.ndarray:
        return np.empty((0, 3), dtype=float) if self.target_realization is None else self.target_realization.position[None, :]
    ...
    @classmethod
    def From_HDF(cls: Type[SingleTargetRadarChannelRealization], group: Group, alpha_device: Device, beta_device: Device) -> SingleTargetRadarChannelRealization:
        parameters = cls._parameters_from_HDF(group)
        parameters["target_realization"] = RadarTargetRealization.from_HDF(group["target_realization"]) if "target_realization" in group else None
...
class MultiTargetRadarChannelRealization(RadarChannelRealization):
    ...
    def __init__(self, alpha_device: Device, beta_device: Device, gain: float, interference_realization: RadarInterferenceRealization | None, target_realizations: Sequence[RadarTargetRealization], interpolation_mode: InterpolationMode = InterpolationMode.NEAREST) -> None:
        """
        Args:
    ...
    def null_hypothesis(self) -> MultiTargetRadarChannelRealization:
        ...
        return MultiTargetRadarChannelRealization(self.alpha_device, self.beta_device, self.gain, null_hypothesis_interference, null_hypothesis_target_realizations, self.interpolation_mode)
    ...
    def to_HDF(self, group: Group) -> None:
        ...
        for t, target_realization in enumerate(self.target_realizations):
            target_realization.to_HDF(HDFSerializable._create_group(group, f"target_realization{t:02d}"))
    ...
    def From_HDF(cls: Type[MultiTargetRadarChannelRealization], group: Group, alpha_device: Device, beta_device: Device) -> MultiTargetRadarChannelRealization:
        ...
        parameters["interference_realization"] = RadarInterferenceRealization.from_HDF(group["interference_realization"]) if "interference_realization" in group else None

        # Deserialize target realizations
        num_target_realizations = group.attrs.get("num_target_realizations", 0)
        target_realizations = [RadarTargetRealization.from_HDF(group[f"target_realization{t:02d}"]) for t in range(num_target_realizations)]

        return MultiTargetRadarChannelRealization(alpha_device, beta_device, target_realizations=target_realizations, **parameters)
...
class SingleTargetRadarChannel(RadarChannelBase[SingleTargetRadarChannelRealization], Serializable):
    ...
    def _realize(self) -> SingleTargetRadarChannelRealization:
        ...
        unit_direction = Direction.From_Spherical(self.target_azimuth, self.target_zenith).view(np.ndarray)
        ...
        # Realize channel
        channel_realization = SingleTargetRadarChannelRealization(self.alpha_device, self.beta_device, self.gain, target_realization, self.interpolation_mode)
        return channel_realization

    def recall_realization(self, group: Group) -> SingleTargetRadarChannelRealization:
        return SingleTargetRadarChannelRealization.From_HDF(group, self.alpha_device, self.beta_device)
...
class MultiTargetRadarChannel(RadarChannelBase[MultiTargetRadarChannelRealization], Serializable):
    ...
    def _realize_interference(self) -> RadarInterferenceRealization | None:
        ...
        return RadarInterferenceRealization(self.attenuate, True) if self.alpha_device is not self.beta_device else None
    ...
    def _realize(self) -> MultiTargetRadarChannelRealization:
        ...
        # Realize channel
        channel_realization = MultiTargetRadarChannelRealization(self.alpha_device, self.beta_device, self.gain, interference_realization, target_realizations, self.interpolation_mode)
        return channel_realization

    def recall_realization(self, group: Group) -> MultiTargetRadarChannelRealization:
        return MultiTargetRadarChannelRealization.From_HDF(group, self.alpha_device, self.beta_device)
