...
class IdealChannelRealization(ChannelRealization):
    ...
    def state(self, transmitter: Device, receiver: Device, delay: float, sampling_rate: float, num_samples: int, num_taps: int) -> ChannelStateInformation:
        # MISO case
        if receiver.antennas.num_receive_antennas == 1:
            spatial_response = np.ones((1, transmitter.antennas.num_receive_antennas), dtype=np.complex_)

        # SIMO case
        elif transmitter.antennas.num_transmit_antennas == 1:
            spatial_response = np.ones((receiver.antennas.num_receive_antennas, 1), dtype=np.complex_)

        # MIMO case
        else:
            spatial_response = np.eye(receiver.antennas.num_receive_antennas, transmitter.antennas.num_transmit_antennas, dtype=np.complex_)

        # Scale response by channel gain
        spatial_response *= np.sqrt(self.gain)

        sampled_state = np.expand_dims(np.repeat(spatial_response[:, :, np.newaxis], num_samples, 2), axis=3)
    ...
    def _propagate(self, signal: Signal, transmitter: Device, receiver: Device, interpolation: InterpolationMode) -> Signal:
        ...
        if transmitter.antennas.num_antennas == 1:
            propagated_samples = np.repeat(signal.samples[[0], :], receiver.antennas.num_antennas, axis=0)

        # Single antenna receiver case
        elif receiver.antennas.num_antennas == 1:
            propagated_samples = np.sum(signal.samples, axis=0, keepdims=True)

        # MIMO case
        else:
            propagated_samples = signal.samples
            if transmitter.antennas.num_antennas < receiver.antennas.num_antennas:
                propagated_samples = np.append(propagated_samples, np.zeros((receiver.antennas.num_antennas - transmitter.antennas.num_antennas, signal.num_samples), dtype=np.complex_), axis=0)
