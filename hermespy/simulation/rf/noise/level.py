...
class NoiseLevel(ScalarDimension, Serializable):
    ...
    def get_power(self, bandwidth: float) -> float:
        ...
        Args:
            bandwidth: Bandwidth of the assumed band in Hz.
...
class ThermalNoise(NoiseLevel):
    """Thermal noise configuration."""

    __temparature: float

    def __init__(self, temparature: float) -> None:
        """
        Args:
            temparature: Absolute temparature in Kelvin.
        """

        # Init attributes
        self.level = temparature
    ...
    def level(self, value: float) -> None:
        if value < 0.0:
            raise ValueError("Absolute temparature must be non-negative.")

        self.__temparature = value

    @override
    def get_power(self, bandwidth: float) -> float:
        return 4 * 50 * Boltzmann * self.level * bandwidth
    ...
    def serialize(self, process: SerializationProcess) -> None:
        process.serialize_floating(self.level, "temparature")

    @classmethod
    @override
    def Deserialize(cls: Type[ThermalNoise], process: DeserializationProcess) -> ThermalNoise:
        return cls(process.deserialize_floating("temparature"))
