...
class PowerLawPhaseNoise(PhaseNoise, Serializable):
    ...
    def __init__(self, flicker_scale: float, white_fm_scale: float, flicker_fm_scale: float, random_walk_fm_scale: float, gauss_cutoff: float, vicinity: float = 0.0, num_flicker_subterms: int = 10) -> None:
        """
        Args:
    ...
    def vicinity(self, value: float) -> None:
        ...
        if value < 0.0:
            raise ValueError("Power law vicinity must be greater or equal to zero")
    ...
    def flicker_scale(self, value: float) -> None:
        ...
        if value < 0.0:
            raise ValueError("Spectral density scale must be greater or equal to zero")
    ...
    def flicker_num_subterms(self, value: int) -> None:
        ...
        if value < 1:
            raise ValueError("Number of flicker subterms must be greater than zero")
    ...
    def flicker_psd(self, frequencies: np.ndarray) -> np.ndarray:
        ...
        nonzero_frequency_indices = np.where(frequencies != 0.0)
        nonzero_frequencies = frequencies[nonzero_frequency_indices]
        psd[np.where(frequencies == 0.0)] = 2 * pi

        y = 1  # Approximation for small vicinities

        for n in range(1, 1 + self.flicker_num_subterms):

            coefficient = (-1) ** (n + 1) * 2 * sin(0.5 * pi * n * self.vicinity) * self.vicinity * (2 * y * self.flicker_scale / self.vicinity) ** n * gamma(n * self.vicinity) / gamma(n)
            psd[nonzero_frequency_indices] += coefficient * np.abs((2 * pi * nonzero_frequencies)) ** (-1 - n * self.vicinity)
    ...
    def flicker_fm_psd(self, frequencies: np.ndarray) -> np.ndarray:
        ...
        K = 16 * pi**2 * sqrt(pi) * (2 * pi) ** (-self.vicinity) * gamma(0.5 * self.vicinity) * self.flicker_fm_scale / (2 - self.vicinity) * gamma(1.5 - 0.5 * self.vicinity)
        ...
        autocorrelation[0] = 0
        autocorrelation[1:] = 0.5 * self.vicinity * K * np.exp(-0.5 * K * delays**2) * delays**2 * np.log(delays)
        autocorrelation_fft = fft(autocorrelation)

        gaussian = sqrt(2 * pi / K) * np.exp(-((2 * pi * frequencies) ** 2) / (2 * K))
    ...
    def random_walk_fm_psd(self, frequencies: np.ndarray) -> np.ndarray:
        ...
        psd[np.where(frequencies == 0.0)] = sqrt(ro) / (2 * pi * sqrt(pi * self.random_walk_fm_scale))
        psd[nonzero_frequency_indices] = sqrt(ro) / (2 * pi * sqrt(pi * self.random_walk_fm_scale)) * np.exp(-ro * (2 * pi * nonzero_frequencies) ** 2 / (16 * pi**4 * self.random_walk_fm_scale))

        quartic_frequency_indices = np.where(np.abs(frequencies) > self.gauss_cutoff)
        quartic_frequencies = frequencies[quartic_frequency_indices]
        psd[quartic_frequency_indices] += self.random_walk_fm_scale * quartic_frequencies**-4

        return psd

    @lru_cache(maxsize=1)
    def _psd(self, sampling_rate: float, num_samples: int) -> np.ndarray:
    ...
    def plot_psds(self, max_frequency: float, num_samples: int) -> plt.Figure:
        ...
        with Executable.style_context():
            ...
            axes[0].semilogy(nonneg_frequencies, self.flicker_psd(frequencies)[nonneg_frequency_indices], color="blue", linestyle="dashed", label="Flicker (Power 1)")
            axes[0].semilogy(nonneg_frequencies, self.white_fm_psd(frequencies)[nonneg_frequency_indices], color="white", linestyle="dashed", label="White FM (Power 2)")
            axes[0].semilogy(nonneg_frequencies, abs(self.flicker_fm_psd(frequencies)[nonneg_frequency_indices]), color="pink", linestyle="dashed", label="Flicker FM (Power 3)")
            axes[0].semilogy(nonneg_frequencies, self.random_walk_fm_psd(frequencies)[nonneg_frequency_indices], color="brown", linestyle="dashed", label="Random Walk FM (Power 4)")
            ...
            axes[1].semilogy(nonneg_frequencies, abs(self._psd(max_frequency, num_samples)[nonneg_frequency_indices]), label="PSD")
            axes[1].axvline(x=f1, color="blue", linestyle="dashed", label="Flicker")
            axes[1].axvline(x=f2, color="white", linestyle="dashed", label="White")
            axes[1].axvline(x=f3, color="pink", linestyle="dashed", label="Flicker FM")
            axes[1].axvline(x=f4, color="brown", linestyle="dashed", label="Random Walk FM")
            axes[1].legend()
