...
class SpecificIsolation(Serializable, Isolation):
    ...
    def __init__(self,
                 ...
                 isolation: Optional[np.ndarray] = None) -> None:
    ...
    def isolation(self, value: Optional[np.ndarray]) -> None:
        ...
        if value.ndim != 2:
            raise ValueError(
                "Isolation specification must be a two dimensional array")
    ...
    def _leak(self, signal: Signal) -> Signal:
        ...
        if self.__isolation.shape[0] != self.device.antennas.num_receive_antennas:
            raise RuntimeError("Number of receiving antennas in isolation specifications ({self.__isolation.shape[0]}) "
