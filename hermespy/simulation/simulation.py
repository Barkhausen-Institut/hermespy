...
class Simulation(Executable, Scenario[SimulatedDevice]):
    ...
    def receive_devices(self, propagation_matrix: List[List[Tuple[Signal, ChannelStateInformation]]]) -> List[Signal]:
        ...
        received_signals: List[Signal] = []
        for device, propagated_signals in zip(self.devices, propagation_matrix):

            baseband_signal = device.receive(propagated_signals)
