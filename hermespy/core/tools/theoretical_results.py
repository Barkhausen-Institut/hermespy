...
from hermespy.modem import CommunicationWaveform, ChirpFSKWaveform, FilteredSingleCarrierWaveform
...
class TheoreticalResults:
    ...
    Attributes:
        ...
        __waveforms (List[type(CommunicationWaveform)]):
    ...
    def __init__(self) -> None:
        ...
        self.__waveforms = [ChirpFSKWaveform, FilteredSingleCarrierWaveform]
        self.__channels = [MultipathFadingChannel, Channel]

        # Generate theory callback lookup table
        self.__theory_grid = np.array(
            [[self.__default_theory for _ in self.__channels] for _ in self.__waveforms]
        )
    ...
    def theory(self, scenario: Scenario, snrs: np.ndarray) -> np.ndarray:
        ...
        for tx_id, transmitter in enumerate(scenario.transmitters):
            for rx_id, receiver in enumerate(scenario.receivers):
                theoretical_results[tx_id, rx_id] = self.theoretical_results(
                    transmitter, receiver, channels[tx_id, rx_id], snrs
                )

        return theoretical_results

    def theoretical_results(
        self, transmitter: Transmitter, receiver: Receiver, channel: Channel, snrs: np.ndarray
    ) -> Optional[Dict[str, Any]]:
        # ToDo: Fix
        return None

        # Currently, only identical waveform generators are theoretically supported
        if isinstance(transmitter.waveform, type(receiver.waveform)):
            return None

        waveform_type = type(transmitter.waveform)
        channel_type = type(channel)

        if waveform_type not in self.__waveforms:
            return None
        ...
        waveform_index = self.__waveforms.index(waveform_type)
    ...
    @staticmethod
    def __default_theory(
        transmitter: Transmitter, receiver: Receiver, channel: Channel, snrs: np.ndarray
    ) -> None:
    ...
    @staticmethod
    def __theory_pskquam_channel(
        transmitter: Transmitter, receiver: Receiver, channel: Channel, snrs: np.ndarray
    ) -> Optional[Dict[str, Any]]:
        bits_in_frame = receiver.num_data_bits_per_frame
        modulation_order = receiver.waveform.modulation_order
        ...
        # M-PSK
            ...
            ber = (
                2
                * stats.norm.sf(
                    np.sqrt(2 * bits_per_symbol * snrs) * np.sin(np.pi / modulation_order)
                )
                / bits_per_symbol
            )
    ...
    @staticmethod
    def __theory_pskquam_stochastic(
    ...
    @staticmethod
    def __theory_chirpfsk_channel(
        transmitter: Transmitter, receiver: Receiver, channel: Channel, ebn0_linear: np.ndarray
    ) -> Optional[Dict[str, Any]]:
        mod_order = receiver.waveform.modulation_order
        ...
        for n in range(2, mod_order + 1):
            ser += (
                (-1) ** n
                / n
                * exp(-(n - 1) * n_bits / n * ebn0_linear)
                * comb(mod_order - 1, n - 1)
            )

        # Bit error rate
        ber = 2 ** (n_bits - 1) / (2**n_bits - 1) * ser
        fer = 1 - (1 - ser) ** receiver.waveform.num_data_chirps
