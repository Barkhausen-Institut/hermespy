...
from .monte_carlo import Artifact, Evaluation, EvaluationResult, Evaluator, GridDimension
...
class ReceivedPowerResult(EvaluationResult):
    ...
    def __init__(self, agerage_powers: np.ndarray) -> None:
        """
        Args:

            average_power: The received power in Watts for each antenna stream.
        """

        # Initialize the base class
        EvaluationResult.__init__(self)

        # Initialize class members
        self.__average_powers = agerage_powers
...
class ReceivedPowerEvaluator(Evaluator):
    ...
    def __init__(self, target: Receiver) -> None:
        """
    ...
    def generate_result(self, grid: Sequence[GridDimension], artifacts: np.ndarray) -> ReceivedPowerResult:
        if self.target.device is None:
            raise RuntimeError("The target has no device to evaluate")

        average_powers = np.zeros((*artifacts.shape, self.target.device.num_receive_antennas), dtype=np.float_)
        ...
        return ReceivedPowerResult(average_powers)
