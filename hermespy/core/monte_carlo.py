...
class ScalarEvaluationResult(EvaluationResult):
    ...
    def __init__(self,
        ...
        self.__grid = grid
        self.__scalar_results = np.empty(section_artifacts.shape, dtype=float)
        ...
        for section_coords in np.ndindex(section_artifacts.shape):
            self.__scalar_results[section_coords] = np.mean([a.to_scalar() for a in section_artifacts[section_coords]])
...
class GridSection(Persistent):
    ...
    def add_samples(self,
        ...
        for sample in samples:
            ...
            #new_artifacts = np.array(sample.artifacts, dtype=object)[::, np.newaxis]
            #self.__artifacts = np.append(self.__artifacts, new_artifacts, axis=1)
...
class MonteCarlo(Generic[MO]):
    ...
    __progress_log_interval = 1                 # Interval between result logs in seconds
    ...
    __console_mode: ConsoleMode                 # Printing behaviour of the simulation during runtime
    __section_block_size: Optional[int]         # Number of samples per section block
    ...
    def simulate(self,
        ...
        if self.__database_caching:
            with (self.console.status("Initializing Database...", spinner='dots') if self.__console_mode == ConsoleMode.INTERACTIVE else nullcontext()):
                ...
                if not 'sample_grid' in database_root:
        ...
        with (self.console.status("Launching Actor Pool...", spinner='dots') if self.__console_mode == ConsoleMode.INTERACTIVE else nullcontext()):
            ...
            task_overhead = 0 # 2  # A little overhead in task submission might speed things up? Not clear atm.
