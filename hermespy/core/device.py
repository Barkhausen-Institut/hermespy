...
class DeviceReception(HDFSerializable):
    """Receive information required by devices"""

    __impinging_signals: List[Signal]

    def __init__(self,
                 impinging_signals: Union[Signal, List[Signal]]) -> None:
        """
        Args:
    ...
    @property
    def impinging_signals(self) -> List[Signal]:
...
class ProcessedDeviceReception(DeviceReception):
    ...
    @classmethod
    def from_HDF(cls: Type[ProcessedDeviceReception], group: Group) -> ProcessedDeviceReception:
...
class Device(ABC, RandomNode):
    ...
    def transmit(self, clear_cache: bool = True) -> DeviceTransmission:
        ...
        operator_transmissions = self.transmitters.get_transmissions(clear_cache)
        for transmission in operator_transmissions:

            if transmission is not None:
                mixed_signal.superimpose(transmission.signal)

        return DeviceTransmission(mixed_signal, operator_transmissions)
