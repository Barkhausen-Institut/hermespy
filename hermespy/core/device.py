...
from typing import Generic, Iterator, Iterable, List, Optional, Tuple, Type, TypeVar, Union
...
class DeviceOutput(HDFSerializable):
    ...
    __emerging_signals: Optional[List[Signal]]
    __mixed_signal: Optional[Signal]
    ...
    def __init__(self,
        ...
        if sampling_rate <= 0.:
            raise ValueError("Device sampling rate must be greater than zero")

        if num_antennas < 1:
            raise ValueError("Number of transmitting device antennas must be greater than zero")

        self.__operator_transmissions = operator_transmissions
        self.__sampling_rate = sampling_rate
        self.__num_antennas = num_antennas
        self.__carrier_frequency = carrier_frequency
        self.__emerging_signals = None
        self.__mixed_signal = None
    ...
    def emerging_signals(self) -> List[Signal]:
        ...
        if self.__emerging_signals is None:
            self.__emerging_signals = [t.signal for t in self.operator_transmissions]
    ...
    def mixed_signal(self) -> Signal:
        ...
        # Generate the mixed signal if it has not been generated yet
...
class ProcessedDeviceInput(DeviceInput):
    ...
    __operator_inputs: List[Tuple[Signal, ChannelStateInformation]]
    __operator_separation: bool

    def __init__(self,
                 impinging_signals: Union[DeviceInput, Signal, List[Signal]],
    ...
    def from_HDF(cls: Type[ProcessedDeviceInput], group: Group) -> ProcessedDeviceInput:
        ...
        # Recall attributes
        operator_separation: bool = group.attrs.get("operator_separation", False)

        return cls(device_input, operator_inputs, operator_separation)
...
class DeviceReception(ProcessedDeviceInput):
    ...
    def __init__(self,
                 impinging_signals: Union[Signal, List[Signal]],
                 operator_inputs: List[Tuple[Signal, ChannelStateInformation]],
        ...
        Args:
            ...
            operator_inputs (bool): Information cached by the device operators.
            operator_separation (bool): Is the operator separation flag enabled?
            operator_receptions (List[Reception]): Information inferred from receive operators.
        """
    ...
    @classmethod
    def From_ProcessedDeviceInput(cls: Type[DeviceReception],
                                   device_input: ProcessedDeviceInput,
                                   operator_receptions: List[Reception]) -> DeviceReception:
        ...
        return cls(device_input.impinging_signals, device_input.operator_inputs, device_input.operator_separation, operator_receptions)
...
class Device(ABC, RandomNode):
    ...
    def transmit(self, clear_cache: bool = True) -> DeviceOutput:
        ...
        return DeviceOutput(operator_transmissions, self.sampling_rate, self.num_antennas, self.carrier_frequency)

    def receive(self, impinging_signals: Union[DeviceInput, Signal, Iterable[Signal]]) -> ProcessedDeviceInput:
        ...
        # Convert argument
        impinging_signals = [impinging_signals] if isinstance(impinging_signals, Signal) else impinging_signals
        impinging_signals: Iterable[Signal] = impinging_signals.impinging_signals if isinstance(impinging_signals, DeviceInput) else impinging_signals
        ...
        # If in operator separation mode, the device requires a dedicated signal model for each operator
        if self.operator_separation:

            # Make sure the number of signals matches the number of receive operators
            if len(impinging_signals) != self.receivers.num_operators:
                raise ValueError(f"Device with enabled operator separation received unexpected number of impinging signal models ({len(impinging_signals)} instead of {self.receivers.num_operators})")

            for receiver, signal in zip(self.receivers, impinging_signals):
                receiver.cache_reception(signal)

            operator_inputs = impinging_signals

        # If operator separation is disabled, the impinging signals are a superposition of all signal models
        else:

            for receiver in self.receivers:
                receiver.cache_reception(superimposed_signal)

            operator_inputs = [superimposed_signal for _ in self.receivers]
        
        return ProcessedDeviceInput(impinging_signals, operator_inputs, self.operator_separation)
