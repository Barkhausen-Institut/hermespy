...
class DeviceTransmission(OperationResult):
    ...
    def from_HDF(cls: Type[DeviceTransmission], group: Group) -> DeviceTransmission:
        ...
        transmissions = [Transmission.from_HDF(
            group[f'transmission_{f:02d}']) for f in range(num_transmissions)]
...
class DeviceReception(OperationResult):
    ...
    def from_HDF(cls: Type[DeviceReception], group: Group) -> DeviceReception:
        ...
        csi = ChannelStateInformation.from_HDF(
            group['csi']) if 'csi' in group else None

        # Recall individual detected frames
        num_receptions = group.attrs.get('num_operator_receptions', 0)
        receptions = [Reception.from_HDF(
            group[f'reception_{f:02d}']) for f in range(num_receptions)]
...
class TransmitterSlot(OperatorSlot[Transmitter]):
    ...
    def get_transmissions(self,
                          clear_cache: bool = True) -> List[Transmission]:
...
class Device(ABC, RandomNode):
    ...
    def __init__(self,
        ...
        self.antennas = UniformArray(
            IdealAntenna(), 5e-3, (1,)) if antennas is None else antennas
    ...
    def power(self, value: float) -> None:
        ...
        if value < 0.:
            raise ValueError(
                "Average signal power must be greater or equal to zero")
    ...
    def position(self, value: Optional[np.ndarray]) -> None:
        ...
        if len(value) > 3:
            raise ValueError(
                "Modem position may have at most three dimensions")
    ...
    def orientation(self, value: Optional[np.ndarray]) -> None:
        ...
        if value.ndim != 1 or len(value) != 3:
            raise ValueError(
                "Modem orientation must be a three-dimensional vector")
