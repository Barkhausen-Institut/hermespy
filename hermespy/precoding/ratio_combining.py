...
class MaximumRatioCombining(SymbolPrecoder, Serializable):
    ...
    def encode(self, symbol_stream: np.ndarray) -> np.ndarray:
        raise NotImplementedError(
            "Maximum ratio combining only supports decoding operations")
    ...
    def decode(self,
        ...
        resulting_symbols = np.sum(
            symbol_stream * stream_responses.conj(), axis=0, keepdims=True)
        resulting_noises = np.sum(
            stream_noises * (np.abs(stream_responses) ** 2), axis=0, keepdims=True)
