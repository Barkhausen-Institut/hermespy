...
class FilteredSingleCarrierWaveform(ConfigurablePilotWaveform):
    ...
    def __init__(self,
        ...
        WaveformGenerator.__init__(
            self, oversampling_factor=oversampling_factor, **kwargs)

        pilot_symbol_sequence = MappedPilotSymbolSequence(
            self.__mapping) if pilot_symbol_sequence is None else pilot_symbol_sequence
        ConfigurablePilotWaveform.__init__(
            self, symbol_sequence=pilot_symbol_sequence, repeat_symbol_sequence=repeat_pilot_symbol_sequence)
    ...
    def num_preamble_symbols(self, value: int) -> None:
        ...
        if value < 0:
            raise ValueError(
                "Nummber of preamble symbols must be greater or equal to zero")
    ...
    def num_postamble_symbols(self, value: int) -> None:
        ...
        if value < 0:
            raise ValueError(
                "Nummber of postamble symbols must be greater or equal to zero")
    ...
    def pilot_signal(self) -> Signal:
        ...
        pilot_symbols[::self.oversampling_factor] = self.pilot_symbols(
    ...
    def map(self, bits: np.ndarray) -> Symbols:
        ...
        symbols[self.num_preamble_symbols+self._num_payload_symbols:
                ] = pilot_symbols[self.num_preamble_symbols + self._num_pilot_symbols:]

        # Assign payload symbols as a mixture of data and pilot symbols
        payload_symbol_slice = symbols[self.num_preamble_symbols:
                                       self.num_preamble_symbols+self._num_payload_symbols]
        payload_symbol_slice[self._data_symbol_indices] = data_symbols
        payload_symbol_slice[self._pilot_symbol_indices] = pilot_symbols[
    ...
    def unmap(self, symbols: Symbols) -> np.ndarray:
        ...
        payload = symbols.raw[:, self.num_preamble_symbols:
                              self.num_preamble_symbols + self._num_payload_symbols, :]
    ...
    def modulate(self, symbols: Symbols) -> Signal:
        ...
        frame = np.zeros(1 + (self._num_frame_symbols - 1) *
                         self.oversampling_factor, dtype=complex)
    ...
    def demodulate(self, signal: np.ndarray) -> Symbols:
        ...
        num_symbols = self.num_preamble_symbols + \
            self.num_postamble_symbols + self._num_payload_symbols
        symbols = filtered_signal[filter_delay:filter_delay +
                                  num_symbols * self.oversampling_factor:self.oversampling_factor]
    ...
    def _equalizer(self, data_symbols: np.ndarray, channel: np.ndarray, noise_var) -> np.ndarray:
        ...
        if self._pulse_correlation_matrix:
            ...
            equalized_symbols = np.matmul(
                isi_equalizer, data_symbols[:, np.newaxis]).flatten()
        else:
    ...
    def _pilot_symbol_indices(self) -> np.ndarray:
        ...
        pilot_indices = np.arange(
            1, 1 + self._num_pilot_symbols) * (1 + self.pilot_rate) - 1
...
class SingleCarrierIdealChannelEstimation(IdealChannelEstimation[FilteredSingleCarrierWaveform], Serializable):
    ...
    def estimate_channel(self, symbols: Symbols) -> StatedSymbols:
        ...
        filter_characteristics = self.waveform_generator._transmit_filter() * \
            self.waveform_generator._receive_filter()
        csi = self._csi().state[:, :, :, [0]]
        filtered_state = np.apply_along_axis(lambda c: np.convolve(
            c, filter_characteristics), axis=2, arr=csi)

        delay = self.waveform_generator._filter_delay
        demodulated_state = filtered_state[:, :, delay:delay+self.waveform_generator.symbols_per_frame *
                                           self.waveform_generator.oversampling_factor:self.waveform_generator.oversampling_factor, [0]]
...
class SingleCarrierLeastSquaresChannelEstimation(Serializable, SingleCarrierChannelEstimation):
    ...
    def estimate_channel(self, symbols: Symbols) -> StatedSymbols:
        ...
        pilot_symbols = symbols.raw[:, num_preamble_symbols:num_preamble_symbols +
                                    num_payload_symbols, 0][:, pilot_symbol_indices]
        postamble_symbols = symbols.raw[:,
                                        num_preamble_symbols + num_payload_symbols:, 0]
        ...
        channel_estimation_stem_indices = np.concatenate((
            ...
            np.arange(num_postamble_symbols) +
            num_preamble_symbols + num_payload_symbols
        ))

        # Interpolate to the whole channel estimation
        channel_estimation_indices = np.arange(
            self.waveform_generator.symbols_per_frame)
        channel_estimation = np.empty(
            (symbols.num_streams, symbols.num_blocks), dtype=complex)
    ...
    def equalize_channel(self,
        ...
        if csi.num_transmit_streams < 2:
            ...
            equalized_symbols = Symbols(
                frame.raw / (csi.state[:, 0, :frame.num_symbols, 0] + 1 / snr))
            return equalized_symbols
...
class RolledOffSingleCarrierWaveform(FilteredSingleCarrierWaveform):
    ...
    def relative_bandwidth(self, value: float) -> None:
        ...
        if value <= 0.:
            raise ValueError(
                "Relative pulse bandwidth must be greater than zero")
...
class RootRaisedCosineWaveform(RolledOffSingleCarrierWaveform, Serializable):
    ...
    def _base_filter(self) -> np.ndarray:
        ...
        impulse_response = np.zeros(
            self.oversampling_factor * self.filter_length)

        # Generate timestamps
        time = np.linspace(-int(.5 * self.filter_length), int(.5 * self.filter_length), self.filter_length *
                           self.oversampling_factor, endpoint=(self.filter_length % 2 == 1)) * self.relative_bandwidth
        ...
        impulse_response[idx] = ((np.sin(np.pi * time[idx] * (1 - self.roll_off)) + 4 * self.roll_off * time[idx] * np.cos(
            np.pi * time[idx] * (1 + self.roll_off))) / (np.pi * time[idx] * (1 - (4 * self.roll_off * time[idx])**2)))
        if np.any(idx_x_by_0):
            impulse_response[idx_x_by_0] = self.roll_off / np.sqrt(2) * ((1 + 2 / np.pi) * np.sin(
                np.pi / (4 * self.roll_off)) + (1 - 2 / np.pi) * np.cos(np.pi / (4 * self.roll_off)))
...
class FMCWWaveform(Serializable, FilteredSingleCarrierWaveform):
    ...
    def _transmit_filter(self) -> np.ndarray:
        ...
        impulse_response[time > self.__true_chirp_duration] = 0.
