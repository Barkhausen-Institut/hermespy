...
class ArtifactTemplate(Generic[AT], Artifact):
    ...
    def __str__(self) -> str:
        ...
        return str(self.__artifact)

    def to_scalar(self) -> AT:
...
class MonteCarloActor(Generic[MO]):
    ...
    @staticmethod
    @abstractmethod
    def sample(investigated_object: MO) -> MO:
...
class MonteCarlo(Generic[MO]):
    ...
    def simulate(self, actor: Type[MonteCarloActor]) -> None:
        ...
        # Generate an actual ray actor from the supplied actor
        @ray.remote(num_cpus=1)
        class Actor(actor, ABC):
            pass
