...
For example, a :class:`SimplexLink<hermespy.modem.modem.SimplexLink>` transmitting a :class:`hermespy.modem.waveform_single_carrier.RootRaisedCosineWaveform` from the first to the second device can be configured as follows:
...
Note that these models might require specifying the linked devices' :meth:`trajectory<hermespy.simulation.animation.Moveable.trajectory>`.
