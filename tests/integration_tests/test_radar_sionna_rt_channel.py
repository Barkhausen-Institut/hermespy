...
from hermespy.radar import Radar, FMCW, CFARDetector, MaxDetector
...
class TestRadarSionna(unittest.TestCase):
    def test_detection_over_sionnaRT(self):
        ...
        device = simulation.new_device(
            ...
            antennas=SimulatedUniformArray(SimulatedIdealAntenna, .5 * speed_of_light / carrier_frequency, [4, 4, 1]),
            bandwidth=3e9,
        ...
        radar.detector = MaxDetector()  # CFARDetector((2, 100), (0, 0), 0.001)
        ...
        for position in positions:
            ...
            for coord_dist_act, coord_dist_exp in zip(np.abs(position), np.abs(radar_reception.cloud.points[0].position)):
                self.assertAlmostEqual(coord_dist_act, coord_dist_exp, delta=radar.waveform.range_resolution(device.bandwidth))
