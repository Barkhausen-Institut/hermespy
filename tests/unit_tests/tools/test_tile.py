...
class TestTile(TestCase):
    ...
    def test_tile_figures(self) -> None:
        ...
        with patch('matplotlib.get_backend') as get_backend_mock, \
            patch('matplotlib.pyplot.get_fignums') as get_fignums_mock, \
            patch('matplotlib.pyplot.figure') as figure_mock, \
            patch('hermespy.tools.tile.get_monitors', return_value=[]):  # No monitors, so the screen geometry is (0, 0, 900, 600')
