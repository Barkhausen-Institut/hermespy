...
class TestSimulationScenario(TestCase):
    ...
    def test_process_inputs_validation(self) -> None:
        ...
        with self.assertRaises(ValueError):
            self.scenario.process_inputs(impinging_signals=[Mock() for _ in range(self.scenario.num_devices)],
                                         trigger_realizations=[Mock() for _ in range(5)])
...
class TestSimulation(TestCase):
    ...
    def test_run(self) -> None:
        ...
        with patch('hermespy.core.monte_carlo.MonteCarloResult.plot', return_value=[mock_figure]), \
    ...
    def test_serialization_dimension_shorthand(self) -> None:
        ...
        self.assertSequenceEqual([1, 2, 3], simulation.dimensions[0].sample_points)
            
    def test_pip_pacakges(self) -> None:
        """Test the pip packages property"""
        
        self.assertCountEqual(["ray", "numpy", "scipy", "ZODB", "matplotlib", "rich", "sparse", "protobuf", "numba"], self.simulation._pip_packages())
