...
from hermespy.core import AntennaMode, ArtifactTemplate, Signal, SignalReceiver, SignalTransmitter
...
class TestSidelobeEvaluator(TestCase):
    ...
    def setUp(self) -> None:
        ...
        devices[0].add_dsp(SignalTransmitter(Signal.Create(np.ones((4,1), dtype=np.complex128), 1e6, 1e8)))
        devices[1].add_dsp(SignalReceiver(1, 1e6))
