...
from hermespy.simulation.rf.noise.model import AWGN, AWGNRealization, NoiseModel, NoiseRealization
...
class _TestNoiseModel(unittest.TestCase):
    ...
    def test_add_noise(self) -> None:
        ...
        for expected_noise_power in powers:
            signal = Signal.Create(np.zeros((1, 1000000), dtype=complex), sampling_rate=1.0)
            ...
            self.assertEqual(signal.num_samples, noisy_signal.num_samples)
