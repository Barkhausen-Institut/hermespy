...
class TestSelectiveLeakageCalibration(TestCase):
    ...
    def test_remove_leakage_validation(self) -> None:
        ...
        transmitted_signal = Signal(np.array([[1, 2, 3, 4, 5]], dtype=np.complex_), 1.)
        received_signal = Signal(np.array([[1, 2, 3, 4, 5]], dtype=np.complex_), 1.)
    ...
    def test_remove_leakage_positive(self) -> None:
        ...
        corrected_positive_signal = self.calibration.remove_leakage(transmitted_signal, received_signal, 3.)
        corrected_negative_signal = self.calibration.remove_leakage(transmitted_signal, received_signal, -3.)

        assert_array_equal(np.array([[-1.+0.j, -1.-1.j,  3.+0.j,  4.+0.j,  5.+0.j]], dtype=np.complex_), corrected_positive_signal.samples)
        assert_array_equal(np.array([[1.+0.j, 2.+0.j,  3.+0.j,  4.+0.j,  5.+0.j]], dtype=np.complex_), corrected_negative_signal.samples)
    ...
    def test_mmse_estimate_validation(self) -> None:
        ...
        device = PhysicalDeviceDummy(carrier_frequency=1e9, sampling_rate=1e8, seed=42, antennas=UniformArray(IdealAntenna, 1e-3, (2, 1, 1)), receive_transmission=False)
        ...
        with self.assertRaises(ValueError):
            _ = SelectiveLeakageCalibration.MMSEEstimate(device, noise_power=np.array([-1.0, 0.]))
        
        with self.assertRaises(ValueError):
            _ = SelectiveLeakageCalibration.MMSEEstimate(device, noise_power=np.array([0.]))
    ...
    def test_mmse_estimate_without_noise_power(self) -> None:
        ...
        device = PhysicalDeviceDummy(carrier_frequency=1e9, sampling_rate=sampling_rate, max_receive_delay=num_samples/sampling_rate, seed=42, antennas=UniformArray(IdealAntenna, 1e-3, (2, 1, 1)), receive_transmission=False)
    ...
    def test_mmse_estimate_with_noise_power(self) -> None:
        ...
        device = PhysicalDeviceDummy(carrier_frequency=1e9, sampling_rate=sampling_rate, max_receive_delay=num_samples/sampling_rate, seed=42, antennas=UniformArray(IdealAntenna, 1e-3, (2, 1, 1)), receive_transmission=False, noise_power=np.zeros(2))
        leakage_model = SelectiveLeakage.Normal(device, num_samples=num_samples)
        device.isolation = leakage_model
