...
class TestEyePlot(HardwareLoopPlotTest, TestCase):
    ...
    def test_update_plot_frames(self) -> None:
        """Test the update plot routine with existing frames"""

        self._test_update_plot()

    def test_update_plot_no_frames(self) -> None:
        """Test the update plot routine without existing frames"""

        with patch('hermespy.modem.modem.CommunicationReception.num_frames', new_callable=PropertyMock) as num_frames_patch:
            num_frames_patch.return_value = 0
            self._test_update_plot()
