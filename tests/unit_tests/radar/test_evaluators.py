...
class TestRadarEvaluator(TestCase):
    ...
    def setUp(self) -> None:
        ...
        channel = SingleTargetRadarChannel(1., 1.)
    ...
    def test_device_inference(self) -> None:
        ...
        channel = SingleTargetRadarChannel(1., 1., alpha_device=alpha_device, beta_device=beta_device)
...
class TestDetectionProbEvaluator(TestCase):
    ...
    def test_scalar_cdf(self) -> None:
        ...
        self.assertEqual(0., self.evaluator._scalar_cdf(0.))
        self.assertEqual(1., self.evaluator._scalar_cdf(1.))
...
class TestRocEvaluationResult(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.result = RocEvaluationResult(grid, self.detection_probabilities, self.false_alarm_probabilities)
        
    def test_title(self) -> None:
        """Title property should return the correct title"""
        
        self.assertEqual('Receiver Operating Characteristics', self.result.title)
...
class TestRootMeanSquareArtifact(TestCase):
    ...
    def test_to_scalar(self) -> None:
        ...
        self.assertEqual(2 * (self.num_errors / self.cummulation) ** .5, self.artifact.to_scalar())
...
class TestRootMeanSquareEvaluation(TestCase):
    ...
    def setUp(self) -> None:
        ...
        for point in self.ground_truth:
            self.pcl.add_point(PointDetection(point, np.zeros(3), 1.))
...
class TestRootMeanSquareError(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.radar.waveform = FMCW()
        self.radar.device = self.device
        self.radar.detector = ThresholdDetector(.1)
