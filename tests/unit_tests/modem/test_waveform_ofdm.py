...
class TestFrameResource(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.elements = [FrameElement(ElementType.DATA, 2), FrameElement(ElementType.REFERENCE, 1), FrameElement(ElementType.NULL, 3)]
...
class TestFrameSymbolSection(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.section = FrameSymbolSection(self.num_repetitions, self.pattern, self.frame, 2)
    ...
    def test_prefix_offset_validation(self) -> None:
        """Prefix offset property setter should raise ValueError on arguments smaller than zero."""

        with self.assertRaises(ValueError):
            self.section.prefix_offset = -1.2
...
class TestOFDMWaveform(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.ofdm = OFDMWaveform(subcarrier_spacing=self.subcarrier_spacing, modem=self.modem, resources=self.resources, structure=self.sections, num_subcarriers=self.num_subcarriers, oversampling_factor=self.oversampling_factor)
    ...
    def test_bit_energy(self) -> None:
        ...
        self.assertEqual(0.25, self.ofdm.bit_energy)
    ...
    def test_power(self) -> None:
        ...
        self.ofdm = OFDMWaveform(subcarrier_spacing=self.subcarrier_spacing, modem=self.modem, resources=[self.resource_a], structure=[self.section_a], num_subcarriers=self.num_subcarriers, oversampling_factor=self.oversampling_factor)
...
class TestSchmidlCoxPilotSection(TestCase):
    ...
    def test_pilot(self) -> None:
        ...
        for num_subcarriers in num_subcarriers_candidates:
            ...
            half_symbol_length = int(0.5 * len(pilot))
...
class TestSchmidlCoxSynchronization(TestCase):
    ...
    def setUp(self) -> None:
        ...
        test_resource = FrameResource(repetitions=10, prefix_ratio=0.01, elements=[FrameElement(ElementType.DATA, repetitions=11), FrameElement(ElementType.REFERENCE, repetitions=1)])
