...
from hermespy.core import Direction, Signal, Transformation
...
class TestClusterDelayLineRealization(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.realization = ClusterDelayLineRealization(self.alpha_device, self.beta_device, self.gain, self.los, self.rice_factor, self.aoa, self.zoa, self.aod, self.zod, self.cluster_delays, self.cluster_delay_spread, self.cluster_powers, self.polarization_transformations)
    ...
    def test_state_skip_tap(self) -> None:
        ...
        state = self.realization.state(self.alpha_device, self.beta_device, 0.0, 1e6, 10, 1)
...
class TestClusterDelayLine(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.alpha_device = SimulatedDevice(antennas=SimulatedUniformArray(SimulatedIdealAntenna, 0.5 * speed_of_light / self.carrier_frequency, (2, 2)), pose=Transformation.From_RPY(pos=np.array([0.0, 0.0, 0.0]), rpy=np.array([0.0, 0.0, 0.0])), carrier_frequency=self.carrier_frequency)

        self.beta_device = SimulatedDevice(antennas=SimulatedUniformArray(SimulatedIdealAntenna, 0.5 * speed_of_light / self.carrier_frequency, (2, 2)), pose=Transformation.From_RPY(pos=np.array([100.0, 0.0, 0.0]), rpy=np.array([0.0, 0.0, 0.0])), carrier_frequency=self.carrier_frequency)

        self.channel = ClusterDelayLine(self.alpha_device, self.beta_device, delay_spread_mean=self.delay_spread_mean, delay_spread_std=self.delay_spread_std, delay_scaling=self.delay_scaling, num_clusters=self.num_clusters, seed=1234)
    ...
    def test_delay_scaling_validation(self) -> None:
        ...
        with self.assertRaises(ValueError):
            self.channel.delay_scaling = -1.0
    ...
    def test_rice_factor_mean_validation(self) -> None:
        ...
        with self.assertRaises(ValueError):
            self.channel.rice_factor_mean = -1.0

        try:
            self.channel.rice_factor_mean = 0.0
    ...
    def test_rice_factor_std_validation(self) -> None:
        ...
        with self.assertRaises(ValueError):
            self.channel.rice_factor_std = -1.0

        try:
            self.channel.rice_factor_std = 0.0
    ...
    def test_cluster_shadowing_std_validation(self) -> None:
        ...
        with self.assertRaises(ValueError):
            self.channel.cluster_shadowing_std = -1.0

        try:
            self.channel.cluster_shadowing_std = 0.0
    ...
    def test_doppler_shift(self) -> None:
        ...
        self.channel.num_clusters = 1
        self.beta_device.velocity = np.array([10.0, 0.0, 0.0])

        sampling_rate = 1e3
        num_samples = 400
        signal = np.outer(np.ones(4, dtype=complex), np.exp(2j * pi * 0.2 * np.arange(num_samples)))
    ...
    def test_spatial_properties(self) -> None:
        ...
        self.alpha_device.antennas = SimulatedUniformArray(SimulatedIdealAntenna, 0.5 * speed_of_light / self.carrier_frequency, (1,))
        ...
        self.beta_device.antennas = SimulatedUniformArray(SimulatedIdealAntenna, 0.5 * speed_of_light / self.carrier_frequency, (8, 8))

        angle_candidates = [(0, 0), (0.25 * pi, 0.5 * pi), (0.5 * pi, 0), (0.5 * pi, 0.25 * pi), (0.5 * pi, 0.5 * pi)]
        range = 1e3
        ...
        for a, (zenith, azimuth) in enumerate(angle_candidates):
            self.channel.seed = 1
            self.alpha_device.position = range * np.array([cos(azimuth) * sin(zenith), sin(azimuth) * sin(zenith), cos(zenith)], dtype=float)
    ...
    def test_delay_spread_std_validation(self) -> None:
        ...
        with self.assertRaises(ValueError):
            self.channel.delay_spread_std = -1.0

        try:
            self.channel.delay_spread_std = 0.0
    ...
    def test_aod_spread_std_validation(self) -> None:
        ...
        with self.assertRaises(ValueError):
            self.channel.aod_spread_std = -1.0

        try:
            self.channel.aod_spread_std = 0.0
    ...
    def test_aoa_spread_std_validation(self) -> None:
        ...
        with self.assertRaises(ValueError):
            self.channel.aoa_spread_std = -1.0

        try:
            self.channel.aoa_spread_std = 0.0
    ...
    def test_zoa_spread_std_validation(self) -> None:
        ...
        with self.assertRaises(ValueError):
            self.channel.zoa_spread_std = -1.0

        try:
            self.channel.zoa_spread_std = 0.0
    ...
    def test_zod_spread_std_validation(self) -> None:
        ...
        with self.assertRaises(ValueError):
            self.channel.zod_spread_std = -1.0

        try:
            self.channel.zod_spread_std = 0.0
    ...
    def test_cross_polarization_power_std_validation(self) -> None:
        ...
        with self.assertRaises(ValueError):
            self.channel.cross_polarization_power_std = -1.0

        try:
            self.channel.cross_polarization_power_std = 0.0
    ...
    def test_cluster_delay_spread_validation(self) -> None:
        ...
        with self.assertRaises(ValueError):
            self.channel.cluster_delay_spread = -1.0

        try:
            self.channel.cluster_delay_spread = 0.0
    ...
    def test_cluster_aod_spread_validation(self) -> None:
        ...
        with self.assertRaises(ValueError):
            self.channel.cluster_aod_spread = -1.0

        try:
            self.channel.cluster_aod_spread = 0.0
    ...
    def test_cluster_aoa_spread_validation(self) -> None:
        ...
        with self.assertRaises(ValueError):
            self.channel.cluster_aoa_spread = -1.0

        try:
            self.channel.cluster_aoa_spread = 0.0
    ...
    def test_cluster_zoa_spread_validation(self) -> None:
        ...
        with self.assertRaises(ValueError):
            self.channel.cluster_zoa_spread = -1.0

        try:
            self.channel.cluster_zoa_spread = 0.0
