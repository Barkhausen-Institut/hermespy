...
from hermespy.simulation.animation import StaticTrajectory
...
from unit_tests.utils import assert_signals_equal, SimulationTestContext  # type: ignore
...
class TestClusterDelayLineSample(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.transmitter = SimulatedDevice(
            ...
            pose=StaticTrajectory(
                Transformation.From_Translation([2, 3, 4]),
                np.array([1, 2, 3]),
            ),
        )
        self.recevier = SimulatedDevice(
            carrier_frequency=self.carrier_frequency,
            bandwidth=self.bandwidth,
            oversampling_factor=1,
            antennas=SimulatedUniformArray(SimulatedIdealAntenna, 1e-3, (2, 1, 1)),
            pose=StaticTrajectory(
                Transformation.From_Translation([5, 6, 7]),
                np.array([4, 5, 6]),
            ),
        )

        self.transmitter_state = self.transmitter.state(0.0)
        self.receiver_state = self.recevier.state(0.0)
    ...
    def test_propagate_state(self) -> None:
        ...
        signal = DenseSignal.FromNDArray(signal_samples, self.bandwidth, self.carrier_frequency)
...
class TestClusterDelayLine(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.carrier_frequency = 1e9
        self.bandwidth = 1e8
        self.alpha_device = SimulatedDevice(carrier_frequency=self.carrier_frequency, bandwidth=self.bandwidth, oversampling_factor=1, pose=Transformation.From_Translation([0, 0, 0]))
        self.beta_device = SimulatedDevice(carrier_frequency=self.carrier_frequency, bandwidth=self.bandwidth, oversampling_factor=1, pose=Transformation.From_Translation([100, 100, 0]))
    ...
    def _test_propagation(self, sample: ClusterDelayLineSample) -> None:
        ...
        num_samples = 100
        transmitted_signal = DenseSignal.FromNDArray(self.rng.normal(size=(sample.num_transmit_antennas, num_samples)) + 1j * self.rng.normal(size=(sample.num_transmit_antennas, num_samples)), self.bandwidth, self.carrier_frequency)
    ...
    def test_expected_scale(self) -> None:
        ...
        unit_energy_signal = DenseSignal.FromNDArray(np.ones((self.alpha_device.num_transmit_antennas, 100)) / 10, self.bandwidth, self.carrier_frequency)
        num_attempts = 100
    ...
    def test_propagation_time_of_flight(self) -> None:
        ...
        signal = DenseSignal.FromNDArray(np.ones((sample.num_transmit_antennas, 1)), self.bandwidth, self.carrier_frequency)
        ...
        delay = np.argmax(np.abs(propagated_signal.view(np.ndarray)) != 0.0)
    ...
    def test_oxygen_absorption(self) -> None:
        ...
        signal = DenseSignal.FromNDArray(np.ones((self.alpha_device.num_transmit_antennas, 1)), self.bandwidth, self.carrier_frequency)
