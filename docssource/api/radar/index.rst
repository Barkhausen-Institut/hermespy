...
we can use the :doc:`Radar<radar.Radar>` class to configure a device
...
and configures a simulation scenario with a single :doc:`FMCW<fmcw>` :doc:`Radar<radar.Radar>` :doc:`Device</api/core/device.Device>`
transmitting at :math:`60~\mathrm{GHz}`.
The transmitted signals are reflected by a single :doc:`Target</api/channel/radar/single>` with a cross section of :math:`1~\mathrm{m}^2` at a distance of half the maximum detectable range.
...
.. toctree::
   ...
   radar.RadarWaveform
   cube.RadarCube
   detection.RadarDetector
   detection.PointDetection
   detection.RadarPointCloud
   evaluators.RadarEvaluator
